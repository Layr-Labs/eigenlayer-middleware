...
import {IStakeRegistryTypes} from "../../src/interfaces/IStakeRegistry.sol";
...
import "../integration/TimeMachine.t.sol";
import "../integration/User.t.sol";

// This is a base contract for integration tests with the legacy RegistryCoordinator
contract IntegrationBaseLegacy is IntegrationConfigLegacy {
    using Strings for *;
    ...
    function assert_HasNoOperatorInfo(User user, string memory err) internal {
        IRegistryCoordinatorTypes.OperatorInfo memory info =
            registryCoordinator.getOperator(address(user));
    ...
    // Add assertions for M2 quorum specific functionality
    function assert_M2QuorumBitmap(uint256 expected, string memory err) internal {
        uint256 actual = registryCoordinator.m2QuorumBitmap();
        assertEq(actual, expected, err);
    }

    function assert_IsM2Quorum(uint8 quorumNumber, bool expected, string memory err) internal {
        bool actual = registryCoordinator.isM2Quorum(quorumNumber);
        assertEq(actual, expected, err);
    }

    function assert_M2QuorumRegistrationDisabled(bool expected, string memory err) internal {
        // This would require checking the isM2QuorumRegistrationDisabled variable,
        // but since it's not exposed via a getter, we'd need to test indirectly
        // through behavior like trying to register and checking for reverts
    }

    function _getOperatorInfo(
        User user
    ) internal view returns (IRegistryCoordinatorTypes.OperatorInfo memory) {
    ...
    function assert_Snap_Unchanged_OperatorInfo(User user, string memory err) internal {
        IRegistryCoordinatorTypes.OperatorInfo memory curInfo = _getOperatorInfo(user);
        IRegistryCoordinatorTypes.OperatorInfo memory prevInfo = _getPrevOperatorInfo(user);
    ...
    /// @dev Check that the staker has `addedShares` additional delegated shares
    /// for each strategy since the last snapshot
    ...
    /// @dev Check that the staker has `removedShares` fewer delegated shares
    /// for each strategy since the last snapshot
    ...
    function _getPrevOperatorInfo(
        User user
    ) internal timewarp returns (IRegistryCoordinatorTypes.OperatorInfo memory) {
