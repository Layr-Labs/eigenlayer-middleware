...
import {BitmapStrings} from "../integration/utils/BitmapStrings.t.sol";
import {IntegrationDeployerLegacy} from "./IntegrationDeployerLegacy.t.sol";
import {
    User, OperatorSetUser, OperatorSetUser_AltMethods
} from "../integration/OperatorSetUser.t.sol";
...
contract Constants {
    ...
    uint32 constant UNLIMITED_OPERATOR_COUNT = 0xFFFFFFFF;
    uint32 constant DEFAULT_KICK_THRESHOLD_BIPS = 10000; // 100%
    uint32 constant DEFAULT_KICK_OPERATOR_BIPS = 10000; // 100%
}

// The M2 quorum configuration parameters
struct M2QuorumParams {
    // The maximum number of operators allowed
    uint32 maxOperatorCount;
    // The threshold for calculating the minimum stake required to kick an operator
    uint32 kickBIPsOfOperatorStake;
    // The threshold for calculating the minimum stake required to remain in a quorum
    uint32 kickBIPsOfTotalStake;
}

contract IntegrationConfigLegacy is IntegrationDeployerLegacy, G2Operations, Constants {
    using Strings for *;
    using BN254 for *;
    ...
    // Default weight parameters for strategies
    mapping(uint8 quorumNumber => IStakeRegistryTypes.StrategyParams[]) strategyParamsPerQuorum;

    // Count of dynamic / static stakers
    mapping(uint8 quorumNumber => uint32 numStakers) numStakersPerQuorum;
    mapping(uint8 quorumNumber => mapping(address operator => uint32 numStakers))
        numStakersPerOperator;

    // Maps quorumNumbers to operator pubkey hashes to the number of stakers delegated to that pubkey
    mapping(uint8 => mapping(bytes32 => uint32)) operatorNumDelegations;

    /// @dev Quorums with empty lookup settings will use these defaults
    mapping(uint8 => IStakeRegistryTypes.StakeType) defaultStakeTypes;
