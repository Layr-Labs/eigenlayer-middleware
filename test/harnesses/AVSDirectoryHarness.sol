...
contract AVSDirectoryHarness is AVSDirectory {
    ...
    function setAvsOperatorStatus(address avs, address operator, OperatorAVSRegistrationStatus status) external {
    ...
    function setIsMember(address avs, address operator, uint32 operatorSetId, bool membershipStatus) external {
        isMember[avs][operator][operatorSetId] = membershipStatus;
    }

    function _registerToOperatorSetsExternal(address avs, address operator, uint32[] calldata operatorSetIds) external {
        _registerToOperatorSets(avs, operator, operatorSetIds);
    }

    function _deregisterFromOperatorSetsExternal(address avs, address operator, uint32[] calldata operatorSetIds) external {
