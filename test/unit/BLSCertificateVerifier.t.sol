...
} from "../../src/interfaces/IBLSCertificateVerifier.sol";
...
contract BLSCertificateVerifierTest is Test {
    ...
    function setUp() public {
        ...
        vm.startPrank(owner);
        verifier = new BLSCertificateVerifier(testOperatorSet, tableUpdater, maxStaleness);
        vm.stopPrank();
    ...
    function createOperatorsWithSplitKeys(
        uint256 pseudoRandomNumber,
        uint256 numSigners,
        uint256 numNonSigners
    )
        ...
        )
    {
        ...
        IBLSTableCalculatorTypes.BN254OperatorInfo[] memory ops =
            new IBLSTableCalculatorTypes.BN254OperatorInfo[](numOperators);
    ...
    function createOperatorSetInfo(
        IBLSTableCalculatorTypes.BN254OperatorInfo[] memory ops
    ...
    function testUpdateOperatorTable() public {
        ...
        IBLSTableCalculatorTypes.BN254OperatorSetInfo memory operatorSetInfo =
            createOperatorSetInfo(operators);
        ...
        assertEq(
            verifier.latestReferenceTimestamp(),
            referenceTimestamp,
            "Reference timestamp not updated correctly"
        );

        // Verify operator set info was stored correctly
        (
            bytes32 storedOperatorInfoTreeRoot,
            uint256 storedNumOps,
            BN254.G1Point memory storedAggPubkey
        ) = verifier.operatorSetInfos(referenceTimestamp);

        assertEq(
            storedOperatorInfoTreeRoot,
            operatorSetInfo.operatorInfoTreeRoot,
            "Operator info tree root not stored correctly"
        );
        assertEq(storedNumOps, operatorSetInfo.numOperators, "Num operators not stored correctly");
        assertEq(
            storedAggPubkey.X,
            operatorSetInfo.aggregatePubkey.X,
            "Aggregate pubkey X not stored correctly"
        );
        assertEq(
            storedAggPubkey.Y,
            operatorSetInfo.aggregatePubkey.Y,
            "Aggregate pubkey Y not stored correctly"
        );
    }
    ...
    function testVerifyCertificateProportion() public {
        ...
        uint96[] memory signedStakes = verifier.verifyCertificate(cert);
        uint256 signedPercentage0 =
            (uint256(signedStakes[0]) * 10000) / uint256(operatorSetInfo.totalWeights[0]);
        uint256 signedPercentage1 =
            (uint256(signedStakes[1]) * 10000) / uint256(operatorSetInfo.totalWeights[1]);
    ...
    function testEjectOperators() public {
        ...
        // Eject the operator
        verifier.ejectOperators(referenceTimestamp, operatorIndicesToEject, witnesses);
    ...
    function testVerifyCertificateAllSigners() public {
        ...
        IBLSCertificateVerifierTypes.BN254Certificate memory cert = IBLSCertificateVerifierTypes
            .BN254Certificate({
            referenceTimestamp: referenceTimestamp,
            messageHash: msgHash,
            signature: signature,
        ...
        assertEq(
            signedStakes[0],
            operatorSetInfo.totalWeights[0],
            "All stake should be signed for type 0"
        );
        assertEq(
            signedStakes[1],
            operatorSetInfo.totalWeights[1],
            "All stake should be signed for type 1"
        );
    }
    ...
    function testInvalidReferenceTimestamp() public {
        ...
        vm.prank(tableUpdater);
        verifier.updateOperatorTable(existingReferenceTimestamp, operatorSetInfo);
