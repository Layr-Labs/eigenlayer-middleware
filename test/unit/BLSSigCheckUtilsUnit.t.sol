...
contract BLSSigCheckUtilsUnitTests is Test {
    ...
    uint256 constant FP_MODULUS = 21888242871839275222246405745257275088696311157297823662689037894645226208583;
    ...
    function test_isOnCurve_invalidPoints() public pure {
        ...
        for (uint256 i = 0; i < invalidPoints.length; i++) {
            assertFalse(
                ...
                string(abi.encodePacked("Invalid point ", vm.toString(i), " should not be on the curve"))
            );
        }
    }
    ...
    function test_isOnCurve_boundaryPoints() public pure {
        ...
        assertFalse(boundaryPoint.isOnCurve(), "Point with x at modulus boundary should not be on curve");
    ...
    function test_isOnCurve_pointAddition() public view {
        ...
        assertTrue(doubled.X != generator.X || doubled.Y != generator.Y, "Doubled point should be different");
    }
    ...
    function test_isOnCurve_invalidXCoordinates() public pure {
        ...
        for (uint256 i = 0; i < invalidXs.length; i++) {
            ...
            if (mulmod(0, 0, FP_MODULUS) != rhs) {
                assertFalse(point1.isOnCurve(), "Point with invalid x and y=0 should not be on curve");
            }
            if (mulmod(1, 1, FP_MODULUS) != rhs) {
                assertFalse(point2.isOnCurve(), "Point with invalid x and y=1 should not be on curve");
            }
        }
    }
    ...
    function test_isOnCurve_specificPoints() public pure {
        ...
        // Another valid point
        testPoints[2].x = 9727523064272218541460723335320998459488975639302513747055235660443850046724;
        testPoints[2].y = 5031696974169251245229961296941447383441169981934237515842977230762345915487;
