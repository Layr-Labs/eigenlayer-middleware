...
contract BLSSigCheckOperatorStateRetrieverUnitTests is MockAVSDeployer {
    ...
    function test_getOperatorState_revert_neverRegistered() public {
        ...
        sigCheckOperatorStateRetriever.getOperatorState(
    ...
    function test_getOperatorState_deregisteredBeforeReferenceBlockNumber() public {
        ...
        (
            uint256 fetchedQuorumBitmap,
            BLSSigCheckOperatorStateRetriever.Operator[][] memory operators
        ) = sigCheckOperatorStateRetriever.getOperatorState(
            registryCoordinator, defaultOperatorId, uint32(block.number)
        );
    ...
    function test_getCheckSignaturesIndices_returnsCorrect() public {
        ...
        BLSSigCheckOperatorStateRetriever.CheckSignaturesIndices memory checkSignaturesIndices =
        sigCheckOperatorStateRetriever.getCheckSignaturesIndices(
            registryCoordinator,
            uint32(block.number),
            BitmapUtils.bitmapToBytesArray(quorumBitmapThree),
        ...
        checkSignaturesIndices = sigCheckOperatorStateRetriever.getCheckSignaturesIndices(
            registryCoordinator,
    ...
    function testGetOperatorState_Valid(
        ...
        for (uint256 i = 0; i < operatorMetadatas.length; i++) {
            ...
            (uint256 quorumBitmap, BLSSigCheckOperatorStateRetriever.Operator[][] memory operators)
            = sigCheckOperatorStateRetriever.getOperatorState(
    ...
    function test_getQuorumBitmapsAtBlockNumber_returnsCorrect() public {
        ...
        uint256[] memory quorumBitmaps = sigCheckOperatorStateRetriever
            .getQuorumBitmapsAtBlockNumber(registryCoordinator, operatorIds, uint32(block.number));
    ...
    function test_getBatchOperatorId_emptyArray() public {
        ...
        bytes32[] memory operatorIds =
            sigCheckOperatorStateRetriever.getBatchOperatorId(registryCoordinator, operators);
    ...
    function test_getBatchOperatorFromId_emptyArray() public {
        ...
        address[] memory operators =
            sigCheckOperatorStateRetriever.getBatchOperatorFromId(registryCoordinator, operatorIds);
