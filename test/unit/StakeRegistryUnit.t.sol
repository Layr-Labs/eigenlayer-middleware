...
contract StakeRegistryUnitTests is MockAVSDeployer, IStakeRegistryEvents {
    ...
    function setUp() public virtual {
        ...
        stakeRegistryImplementation =
            new StakeRegistryHarness(IRegistryCoordinator(address(registryCoordinator)), delegationMock);

        stakeRegistry = StakeRegistryHarness(
            address(new TransparentUpgradeableProxy(address(stakeRegistryImplementation), address(proxyAdmin), ""))
        );
        cheats.stopPrank();
    ...
    function _initializeQuorum(uint96 minimumStake) internal {
        ...
        strategyParams[0] = IStakeRegistry.StrategyParams(
            IStrategy(address(uint160(uint256(keccak256(abi.encodePacked(quorumNumber)))))), uint96(WEIGHTING_DIVISOR)
        );
    ...
    function _fuzz_setupRegisterOperator(uint192 fuzzy_Bitmap, uint16 fuzzy_addtlStake)
        internal
        returns (RegisterSetup memory)
    {
    ...
    function _fuzz_setupRegisterOperators(uint192 fuzzy_Bitmap, uint16 fuzzy_addtlStake, uint256 numOperators)
        internal
        returns (RegisterSetup[] memory)
    {
    ...
    /// The operator's weight is set to the quorum's minimum, plus fuzzy_addtlStake (overflows are skipped)
    /// This function guarantees at least one quorum, and any quorums returned are initialized
    function _fuzz_setupDeregisterOperator(uint192 registeredFor, uint192 fuzzy_toRemove, uint16 fuzzy_addtlStake)
        internal
        returns (DeregisterSetup memory)
    {
    ...
    function _fuzz_setupUpdateOperatorStake(uint192 registeredFor, int8 fuzzy_Delta)
        internal
        returns (UpdateSetup memory)
    {
        ...
        for (uint256 i = 0; i < minimumStakes.length; i++) {
            ...
            if (fuzzy_Delta > 0) {
                ...
                assertEq(
                    endingWeights[i], minimumStakes[i], "_fuzz_setupUpdateOperatorStake: invalid delta during setup"
                );
            }
        ...
        uint96 stakeDeltaAbs = fuzzy_Delta < 0 ? uint96(-int96(fuzzy_Delta)) : uint96(int96(fuzzy_Delta));
    ...
    function _fuzz_setupUpdateOperatorStakes(uint8 numOperators, uint192 registeredFor, int8 fuzzy_Delta)
        internal
        returns (UpdateSetup[] memory)
    {
    ...
    function _isUnchanged(IStakeRegistry.StakeUpdate memory prev, IStakeRegistry.StakeUpdate memory cur)
        internal
        pure
        returns (bool)
    {
    ...
    function _getLatestStakeUpdates(bytes32 operatorId, bytes memory quorumNumbers)
        internal
        view
        returns (IStakeRegistry.StakeUpdate[] memory)
    {
        IStakeRegistry.StakeUpdate[] memory stakeUpdates = new IStakeRegistry.StakeUpdate[](quorumNumbers.length);
    ...
    function _getStakeHistoryLengths(bytes32 operatorId, bytes memory quorumNumbers)
        internal
        view
    ...
    function _getTotalStakeHistoryLengths(bytes memory quorumNumbers) internal view returns (uint256[] memory) {
...
contract StakeRegistryUnitTests_Config is StakeRegistryUnitTests {
    ...
    function testFuzz_initializeQuorum_Revert_WhenInvalidArrayLengths(uint8 quorumNumber, uint96 minimumStake) public {
     ...
     * strategy addresses.
     */
    function testFuzz_initializeQuorum(uint8 quorumNumber, uint96 minimumStake, uint96[] memory multipliers) public {
        cheats.assume(quorumNumber >= nextQuorum);
        ...
        assertEq(
            stakeRegistry.strategyParamsLength(quorumNumber), strategyParams.length, "invalid strategy params length"
        );
        for (uint256 i = 0; i < strategyParams.length; i++) {
    ...
    function testFuzz_setMinimumStakeForQuorum(uint8 quorumNumber, uint96 minimumStakeForQuorum)
        public
     ...
     * quorumNumber.
     */
    function testFuzz_addStrategies(uint8 quorumNumber, uint96[] memory multipliers)
        public
        fuzzOnlyInitializedQuorums(quorumNumber)
    {
    ...
    function testFuzz_removeStrategies_Revert_WhenInvalidQuorum(uint8 quorumNumber, uint256[] memory indicesToRemove)
        public
    {
        // quorums [0,nextQuorum) are initialized, so use an invalid quorumNumber
        cheats.assume(quorumNumber >= nextQuorum);
    ...
    function testFuzz_removeStrategies_Revert_WhenEmptyStrategiesToRemove(uint96 minimumStake, uint8 numStrategiesToAdd)
        public
    {
        cheats.assume(0 < numStrategiesToAdd && numStrategiesToAdd <= MAX_WEIGHING_FUNCTION_LENGTH);
        uint8 quorumNumber = _initializeQuorum(minimumStake, numStrategiesToAdd);
     ...
     */
    function testFuzz_removeStrategies(uint96 minimumStake, uint8 numStrategiesToAdd, uint8 numStrategiesToRemove)
        public
    {
        cheats.assume(0 < numStrategiesToAdd && numStrategiesToAdd <= MAX_WEIGHING_FUNCTION_LENGTH);
     ...
     */
    function testFuzz_modifyStrategyParams(uint8 numStrategiesToAdd, uint8 numStrategiesToModify) public {
        cheats.assume(0 < numStrategiesToAdd && numStrategiesToAdd <= MAX_WEIGHING_FUNCTION_LENGTH);
...
contract StakeRegistryUnitTests_Register is StakeRegistryUnitTests {
     ...
     */
    function testFuzz_registerOperator_SingleOperator_SingleBlock(uint192 quorumBitmap, uint16 additionalStake)
        public
    {
    ...
    function testFuzz_registerOperator_MultiOperator_SingleBlock(
        ...
        for (uint256 i = 0; i < numOperators; i++) {
            ...
            /// Check results
            assertTrue(
                resultingStakes.length == setup.quorumNumbers.length, "invalid return length for operator stakes"
            );
            assertTrue(totalStakes.length == setup.quorumNumbers.length, "invalid return length for total stakes");
            for (uint256 j = 0; j < setup.quorumNumbers.length; j++) {
                ...
                // Check stake increase from fuzzed input
                assertEq(
                    resultingStakes[j], newOperatorStakes[j].stake, "did not add additional stake to operator correctly"
                );
                // Check this is the first entry in the operator stake history
        ...
        for (uint256 i = 0; i < quorumNumbers.length; i++) {
            ...
            assertEq(
                newTotalStakes[i].updateBlockNumber, uint32(block.number), "incorrect total stake next update block"
            );
        }
    }
    ...
    function testFuzz_registerOperator_MultiOperator_MultiBlock(
        ...
        for (uint256 i = 1; i <= totalBlocks; i++) {
            ...
            for (uint256 j = 0; j < quorumNumbers.length; j++) {
                ...
                assertTrue(
                    prevUpdate.stake < newTotalStakes[j].stake, "previous update should have lower stake than latest"
                );
                assertEq(
...
contract StakeRegistryUnitTests_Deregister is StakeRegistryUnitTests {
     ...
     */
    function testFuzz_deregisterOperator_SingleOperator_SingleBlock(uint192 quorumsToRemove, uint16 additionalStake)
        public
    {
        ...
        for (uint256 i = 0; i < setup.registeredQuorumNumbers.length; i++) {
            ...
            if (deregistered) {
                ...
                assertEq(
                    newTotalStake.updateBlockNumber, uint32(block.number), "total stake has incorrect update block"
                );
        ...
        for (uint256 i = 0; i < numOperators; i++) {
            ...
            for (uint256 j = 0; j < registeredQuorums.length; j++) {
                ...
                if (deregistered) {
                    ...
                    assertEq(
                        newOperatorStake.nextUpdateBlockNumber, 0, "operator stake has incorrect next update block"
                    );
                    assertEq(
                        newTotalStake.updateBlockNumber, uint32(block.number), "total stake has incorrect update block"
                    );
    ...
    function testFuzz_deregisterOperator_MultiOperator_MultiBlock(
        ...
        for (uint256 i = 1; i <= totalBlocks; i++) {
            ...
            for (uint256 j = 0; j < registeredQuorums.length; j++) {
                ...
                // Validate previous entry was updated correctly
                assertTrue(
                    prevUpdate.stake > newTotalStakes[j].stake, "previous update should have higher stake than latest"
                );
                assertEq(
                    prevUpdate.updateBlockNumber + 1,
                    newTotalStakes[j].updateBlockNumber,
...
contract StakeRegistryUnitTests_StakeUpdates is StakeRegistryUnitTests {
    ...
    function testFuzz_updateOperatorStake_SingleOperator_SingleBlock(int8 stakeDelta) public {
        UpdateSetup memory setup =
            _fuzz_setupUpdateOperatorStake({registeredFor: initializedQuorumBitmap, fuzzy_Delta: stakeDelta});
        ...
        for (uint256 i = 0; i < setup.quorumNumbers.length; i++) {
            ...
            if (endingWeight > minimumStake) {
                ...
                assertEq(
                    prevTotalStake.stake - stakeRemoved, newTotalStake.stake, "failed to remove delta from total stake"
                );
     ...
     */
    function testFuzz_updateOperatorStake_MultiOperator_SingleBlock(uint8 numOperators, int8 stakeDelta) public {
        cheats.assume(numOperators > 1 && numOperators < 20);
        ...
        for (uint256 i = 0; i < quorumNumbers.length; i++) {
            ...
            if (endingWeight > minimumStake) {
                ...
                assertEq(
                    initialTotalStake.stake + stakeAdded, finalTotalStake.stake, "failed to add delta for all operators"
                );
            } else if (endingWeight < minimumStake) {
                ...
                assertTrue(
                    _isUnchanged(initialTotalStake, finalTotalStake), "neutral stake delta should result in no change"
                );
            }
        }
    }
     ...
     */
    function testFuzz_updateOperatorStake_SingleOperator_MultiBlocknumberChecks(uint8 totalBlocks, int8 stakeDelta)
        public
    {
            ...
            for (uint256 i = 0; i < setup.quorumNumbers.length; i++) {
                ...
                if (endingWeight > minimumStake) {
                    ...
                    if (prevOperatorStake.stake >= minimumStake) {
                        ...
                        assertEq(
                            prevTotalStakes[i].stake, newTotalStakes[i].stake, "total stake should remain unchanged"
                        );
                    }
                } else {
                    // Check that no update occurs if weight remains the same
...
contract StakeRegistryUnitTests_weightOfOperatorForQuorum is StakeRegistryUnitTests {
     ...
     */
    function test_weightOfOperatorForQuorum(address operator, uint96[] memory multipliers, uint96[] memory shares)
        public
    {
        cheats.assume(0 < multipliers.length && multipliers.length <= MAX_WEIGHING_FUNCTION_LENGTH);
        ...
        for (uint256 i = 0; i < strategyParams.length; i++) {
            multipliers[i] =
                uint96(_randUint({rand: bytes32(uint256(multipliers[i])), min: 0, max: 1000 * WEIGHTING_DIVISOR}));
            ...
            strategyParams[i] = IStakeRegistry.StrategyParams(strat, uint96(WEIGHTING_DIVISOR) + multipliers[i]);
        }
        cheats.prank(address(registryCoordinator));
        uint8 quorumNumber = nextQuorum;
    ...
    function test_weightOfOperatorForQuorum_3Strategies(address operator, uint96[3] memory shares) public {
