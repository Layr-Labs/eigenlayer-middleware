...
contract RegistryCoordinatorMigrationUnit is MockAVSDeployer, IServiceManagerBaseEvents {
    ...
    function test_updateOperatorsForQuorumsAfterDirectUnregister() public {
        vm.prank(proxyAdmin.owner());
        proxyAdmin.upgrade(
            TransparentUpgradeableProxy(payable(address(avsDirectory))), address(avsDirectoryMock)
        );
        ...
        /// TODO: Create quorum, register, advance block, force deregister, and updatestake
        uint32[] memory operatorSetsToUnregister;
        ...
        bool operatorIsUnRegistered = !avsDirectory.isMember(
            operators[0],
        ...
        /// TODO:
        uint256 preNumOperators;
        uint256 postRegisteredOperators;
    ...
    function test_deregister_afterMigration() public {
        ...
        bytes32[] memory registeredOperators =
            indexRegistry.getOperatorListAtBlockNumber(defaultQuorumNumber, uint32(block.number));
        address[] memory registeredOperatorAddresses = new address[](registeredOperators.length);
        for (uint256 i = 0; i < registeredOperators.length; i++) {
            registeredOperatorAddresses[i] =
                registryCoordinator.blsApkRegistry().pubkeyHashToOperator(registeredOperators[i]);
        }
        ...
        bool isOperatorRegistered = avsDirectory.isMember(
            operatorToDeregister,
            IAVSDirectory.OperatorSet({
                avs: address(serviceManager),
                operatorSetId: defaultQuorumNumber
            })
        ...
        isOperatorRegistered = avsDirectory.isMember(
            operatorToDeregister,
            IAVSDirectory.OperatorSet({
                avs: address(serviceManager),
                operatorSetId: defaultQuorumNumber
