...
contract SlashEscrowFactoryMock is ISlashEscrowFactory {
    ...
    function initializeParams(address _strategyManager, uint256 _escrowDelay) external {}
    ...
    function getPendingEscrows()
        external
        view
        returns (OperatorSet[] memory, bool[] memory, uint256[][] memory, uint32[][] memory)
    {
    ...
    function getEscrowCompleteBlock(
        OperatorSet calldata operatorSet,
        uint256 slashId
