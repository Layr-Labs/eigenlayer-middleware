...
contract Operators is Test {
    ...
    function getOperatorPubkeyG2(uint256 index) public view returns (BN254.G2Point memory) {
        BN254.G2Point memory pubkey = BN254.G2Point({
            X: [readUint(operatorConfigJson, index, "PubkeyG2.X.A1"), readUint(operatorConfigJson, index, "PubkeyG2.X.A0")],
            Y: [readUint(operatorConfigJson, index, "PubkeyG2.Y.A1"), readUint(operatorConfigJson, index, "PubkeyG2.Y.A0")]
        });
        return pubkey;
    }
