 ...
 */
contract ECDSAIndexRegistry is Initializable, IECDSAIndexRegistry {
    /// @notice The value that is returned when an operator does not exist at an index at a certain block
    address public constant OPERATOR_DOES_NOT_EXIST_ID = address(0);
    ...
    mapping(uint8 => mapping(address => uint32)) public currentOperatorIndex;
    /// @notice maps quorumNumber => operatorIndex => historical operator ids at that index
    mapping(uint8 => mapping(uint32 => OperatorUpdate[]))
        internal _operatorIndexHistory;
    ...
    *******************************************************************************/
    ...
    function registerOperator(
        ...
        uint32[] memory numOperatorsPerQuorum = new uint32[](
        ...
        for (uint256 i = 0; i < quorumNumbers.length; i++) {
            ...
            require(
                historyLength != 0,
                "IndexRegistry.registerOperator: quorum does not exist"
            );
    ...
    function deregisterOperator(
        ...
        for (uint256 i = 0; i < quorumNumbers.length; i++) {
            ...
            uint32 operatorIndexToRemove = currentOperatorIndex[quorumNumber][
                operatorId
            ];
            ...
            address lastOperatorId = _popLastOperator(
                quorumNumber,
                newOperatorCount
            );
    ...
    *******************************************************************************/
    ...
    function _increaseOperatorCount(
        ...
        if (
            _operatorIndexHistory[quorumNumber][newOperatorCount - 1].length ==
            0
        ) {
    ...
    function _popLastOperator(
        ...
        OperatorUpdate storage lastUpdate = _latestOperatorIndexUpdate(
            quorumNumber,
            operatorIndex
        );
    ...
    function _latestOperatorIndexUpdate(
        ...
        uint256 historyLength = _operatorIndexHistory[quorumNumber][
            operatorIndex
        ].length;
        return
            _operatorIndexHistory[quorumNumber][operatorIndex][
                historyLength - 1
            ];
    }
    ...
    function _operatorCountAtBlockNumber(
        ...
        require(
            historyLength != 0,
            "IndexRegistry._operatorCountAtBlockNumber: quorum does not exist"
        );

        // Loop backwards through the total operator history
        for (uint256 i = 0; i < historyLength; i++) {
            uint256 listIndex = (historyLength - 1) - i;
            QuorumUpdate memory quorumUpdate = _operatorCountHistory[
                quorumNumber
            ][listIndex];
    ...
    function _operatorIdForIndexAtBlockNumber(
        ...
        for (uint256 i = 0; i < historyLength; i++) {
            uint256 listIndex = (historyLength - 1) - i;
            OperatorUpdate memory operatorIndexUpdate = _operatorIndexHistory[
                quorumNumber
            ][operatorIndex][listIndex];
            // Look for the first update that began before or at `blockNumber`
    ...
    *******************************************************************************/
    ...
    function getOperatorListAtBlockNumber(
        ...
        uint32 operatorCount = _operatorCountAtBlockNumber(
            quorumNumber,
            blockNumber
        );
