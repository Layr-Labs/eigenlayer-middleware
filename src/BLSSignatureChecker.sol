...
contract BLSSignatureChecker is IBLSSignatureChecker {
    ...
    // We only use one quorum number in Aligned, so we set it to 0 here instead of passing it around
    ...
    function checkSignatures(
        ...
        require(
            ALIGNED_QUORUM_NUMBER.length != 0,
            "BLSSignatureChecker.checkSignatures: empty quorum input"
        );

        require(
            (ALIGNED_QUORUM_NUMBER.length == params.quorumApks.length) &&
                (ALIGNED_QUORUM_NUMBER.length ==
                    params.quorumApkIndices.length) &&
                (ALIGNED_QUORUM_NUMBER.length ==
                    params.totalStakeIndices.length) &&
                (ALIGNED_QUORUM_NUMBER.length ==
                    params.nonSignerStakeIndices.length),
        ...
        stakeTotals.totalStakeForQuorum = new uint96[](
            ALIGNED_QUORUM_NUMBER.length
        );
        stakeTotals.signedStakeForQuorum = new uint96[](
            ALIGNED_QUORUM_NUMBER.length
        );
