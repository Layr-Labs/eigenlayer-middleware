...
interface IBLSPublicKeyCompendium {
     ...
     */
    // TODO: for now commenting so as not to break all mocks and tests
    //       once we are happy with these mappings then remove the old ones and update the mocks/tests
    // function operatorToG1PubkeyX(
    //     address operator
    // ) external view returns (uint256);

    // function operatorToG1PubkeyY(
    //     address operator
    // ) external view returns (uint256);
