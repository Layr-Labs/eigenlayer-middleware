...
import {EORegistryCoordinator, IEORegistryCoordinator, IEOStakeRegistry, IPauserRegistry} from "../../../src/EORegistryCoordinator.sol";
...
// forge script script/deploy/holesky/Holesky_DeployEOMiddlewareContracts.s.sol --rpc-url $HOLESKY_RPC_URL --private-key $PRIVATE_KEY //--broadcast -vvvv
contract Holesky_DeployEOMiddlewareContracts is
    Utils,
    ExistingDeploymentParser
{
    ...
    string public outputPath =
        string.concat(
            "script/deploy/holesky/output/eoracle_middleware_deployment_data_holesky.json"
        );
        ...
        )
    {
        ...
        eoracleUpgrader = stdJson.readAddress(
            config_data,
        ...
        )
    {
        ...
        stakeRegistryImplementation = new EOStakeRegistry(
            registryCoordinator,
            delegationManager
        );
        blsApkRegistryImplementation = new EOBLSApkRegistry(
            registryCoordinator
        );
        ...
        registryCoordinatorImplementation = new EORegistryCoordinator(
            serviceManager,
            stakeRegistry,
            blsApkRegistry,
            indexRegistry
    ...
    function _initEORegistryCoordinator(
        ProxyAdmin _proxyAdmin,
        IEORegistryCoordinator _registryCoordinator,
        EORegistryCoordinator _registryCoordinatorImplementation,
    ...
    function _verifyInitalizations(string memory config_data) internal {
        ...
        require(
            serviceManager.owner() == eoracleOwner,
            "serviceManager.owner() != eoracleOwner"
        );

        require(
            registryCoordinator.owner() == eoracleOwner,
            "registryCoordinator.owner() != eoracleOwner"
        );
        require(
